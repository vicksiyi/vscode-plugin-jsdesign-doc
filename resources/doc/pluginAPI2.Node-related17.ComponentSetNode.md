# 变体节点 ComponentSetNode

变体就是引用组件的集合，它的所有子节点都是`ComponentNode`，且必须存在子节点。
...
变体的节点类型，由字符串`'COMPONENT_SET'`表示。



#### clone(): ComponentSetNode

复制变体为新的变体，非实例组件集合。默认情况下，复制的节点将成为`jsDesign.currentPage`的子节点。



#### defaultVariant: ComponentNode [readonly]

当前变体的默认变量组件，在空间上是变体内部最左上角的引用组件，对应从设计库中拖入变体时插入的实例组件。
