...
引用组件是「即时设计」中非常重要的一类元素，它具有极高的复用性，且除了在当前文件中使用外，也经常用于团队共享设计库，更多相关操作可以查看 帮助文档 。
...
引用组件的节点类型，由字符串`'COMPONENT'`表示。



#### clone(): ComponentNode

复制引用组件为新的引用组件，非实例组件。默认情况下，复制的节点将成为`jsDesign.currentPage`的子节点。
...
当前组件节点对应的变体分类和变量值，如果未被合并为变体，则为`null`。