...
const SUFFIX = '.md';
export const docFolderPath = path.join(__filename, '..', '..', 'resources', 'doc');
...
export const fileNameHandler = (docPath: string): string => {
	return docPath.split("/").splice(-4, 4).join('') + SUFFIX;
}
 ...
 * @returns 
 */
function getDocKeyVal(obj: any, docRes: any) {
	if (!obj) return;
	const _keys = Object.keys(obj) ?? [];
	_keys?.forEach(_key => {
		if (typeof obj[_key] === 'string') docRes[_key] = obj[_key];
		else getDocKeyVal(obj[_key], docRes);
	})
}
 ...
 * @returns 
 */
export function getDocFlat() {
	let _docFlat: keyStrAndValStr = {};
	getDocKeyVal(doc, _docFlat);
	return _docFlat;
}
 ...
 * @returns 
 */
export function generateSearchContent(_searchResult: keyStrAndValStr): string {
	let md: string[] = [];
	let _keys = Object.keys(_searchResult) ?? [];
	md.push('# 搜索结果');
	if (_keys.length) {
		_keys.forEach(_key => md.push(`#### [${_key}](${_searchResult[_key]})`));
	} else {
		md.push('当前搜索为空');
	}
	return md.join('\n');
